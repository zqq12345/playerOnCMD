...
public class ConsoleProgressBar {
    ...
    public void show(long value) {
        ...
        draw(len, rate,minimum);
        if (minimum == maximum) {
            afterComplete();
        }
    }

    private void draw(long len, float rate,long minimum) {
        for (int i = 0; i < len; i++) {
            System.out.print(showChar);
        }
        System.out.print(' ');
        System.out.print(minusFormat(minimum)+"("+format(rate)+")");
    }

    private static String minusFormat(long time) {
        if (time <= 59) {
            return time+"s";
        }
    ...
    public static void main(String[] args) throws InterruptedException {
        ConsoleProgressBar cpb = new ConsoleProgressBar(0, 1000, 50, '.');
